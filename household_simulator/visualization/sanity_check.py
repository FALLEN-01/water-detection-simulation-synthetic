"""
Sanity-check a generated household flow dataset (visualization package copy).

This is a lightweight CLI wrapper around the same checks used in
`preprocessing/sanity_check.py`, kept here for convenience when working inside
`household_simulator/visualization/`.
"""
...
# CONFIG
# ======================================================
DEFAULT_CSV_PATH = "household_flow_365d_updated.csv"

# ======================================================
# LOAD DATA
# ======================================================
def run(csv_path: str) -> None:
    """Execute the sanity-check report for a given CSV path."""
...
# ======================================================
    print("\n=== PEAK / NIGHT RATIO ===")
    morning = df[(df["timestamp"].dt.hour >= 6) & (df["timestamp"].dt.hour <= 9)][
        "flow_lpm"
    ]
...
# Warnings (plausible but notable)
    ...
    if fail == 0 and warn == 0:
        ...
        print(f"DATA PASSES WITH WARNINGS ({warn})")
    else:
        print(f"DATA FAILS ({fail} critical issues, {warn} warnings)")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run household synthetic data sanity checks.")
    parser.add_argument("--csv", default=DEFAULT_CSV_PATH, help="Path to flow CSV")
